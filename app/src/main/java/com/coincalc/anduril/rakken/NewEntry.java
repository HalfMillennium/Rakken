...
public class NewEntry extends AppCompatActivity {
    ...
    public void submit(View view, final String display)
    {
        ...
        if(!content.getText().toString().replaceAll(" ", "").equals("")) {
            final DatabaseReference ref = FirebaseDatabase.getInstance().getReference(), ref2 = FirebaseDatabase.getInstance().getReference();
            ...
            ref.child("stories").child(storyName).child("contribs").addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {
                    String curr = dataSnapshot.getValue(String.class);

                    ref2.child("stories").child(storyName).child("contribs").setValue(curr + "," + display);
                    ref2.push();
                }
